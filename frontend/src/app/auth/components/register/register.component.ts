...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form.get('username')?.valueChanges.pipe(
      ...
      if (response) {
        this.usernameExists = response.exists;  // API response contains 'exists' property
      } else {
  ...
  onSubmit(): void {
    if (this.form.valid) {
      ...
      this.authService.register(this.form.value.username,this.form.value.password,'patient').subscribe(
        (response) => {
        ...
        (error) => {
          ...
          if (error.status === 400) {
            ...
            // Handle other errors
            this.form.setErrors({ serverError: 'Registration failed. Please try again.' });
          }
        }
      );
